...
Input to the B-GREAT method should come from two dataframes, *data* (n x p) and *meta* (p x k). The index of *data* is assumed to be the time column. Each row of *meta* should correspond to a single column of *data*, e.g. the number of columns in *data* should equal the number of rows in *meta*. 
